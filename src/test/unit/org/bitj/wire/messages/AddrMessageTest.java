...
public class AddrMessageTest extends BaseTest {
  ...
  static AddrMessage MSG = new AddrMessage(
    ImmutableSet.of(
  ...
  public void deserializePayload() throws Exception {
    AddrMessage msg = AddrMessage.deserializePayload(bitcoinStream(MSG_BYTES));
    assertEquals(msg, MSG);
  }

  @Test(expectedExceptions = AddrMessage.TooMany.class)
  public void deserializePayload_WhenToManyAddresses() throws Exception {
    byte[] maliciousBytes = Wire.unsignedIntToVarBytes(1001);
    AddrMessage.deserializePayload(bitcoinStream(maliciousBytes));
  }
  ...
  public void toStringImplemented() throws Exception {
    assertTrue(MSG.toString().contains("AddrMessage{"));
  }
